    ...
    pub(super) async fn move_package_verify(
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            for entry in exclude.as_deref().unwrap_or(&[]) {
                ...
                if !exists {
                    ...
                    return Err(rmcp::ErrorData::invalid_params(
                        format!(
                            "exclude entry `{}` does not match any target {}",
                            entry, kind
                        ),
                        None,
                    ));
            ...
            let exclude_entries: Vec<&str> = exclude
                ...
                .collect();
            let func_excluded =
                |m: &move_model::model::ModuleEnv, func: &move_model::model::FunctionEnv| -> bool {
            ...
            match prover_result {
                ...
                Err(e) => {
                    ...
                    let infrastructure = reported_errors == 0 && !is_prover_timeout(&diag_text);
                    ...
                    let msg = if !diag_text.is_empty() {
                        data.set_diagnostics(DiagnosticSource::Verifier, vec![diag_text.clone()]);
                        let attribution = if attribute_timeouts && is_prover_timeout(&diag_text) {
