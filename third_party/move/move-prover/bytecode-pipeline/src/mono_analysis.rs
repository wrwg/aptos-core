...
pub struct MonoInfo {
    ...
    /// Per-verification-root semantic slices. The first implementation records
    /// concrete struct instances, which are used to localize their validity and
    /// equality definitions in the Boogie backend.
...
impl MonoAnalysisProcessor {
    ...
    fn compute(
        ...
        let mut analyzer = Analyzer {
            ...
            verification_roots: BTreeSet::new(),
            selected_root: root,
        };
...
struct Analyzer<'a> {
    ...
    verification_roots: BTreeSet<VerificationRoot>,
...
impl Analyzer<'_> {
    ...
    fn analyze_verification_root(&mut self, root: VerificationRoot, target: FunctionTarget<'_>) {
        self.verification_roots.insert(root.clone());
    ...
    fn analyze_fun(&mut self, target: FunctionTarget<'_>) {
        ...
        if self.inst_opt.is_none() && self.selected_root.is_none() {
            ...
            for fun_inst in all_insts {
                self.verification_roots.insert(VerificationRoot {
                    fun: target.func_env.get_qualified_id(),
