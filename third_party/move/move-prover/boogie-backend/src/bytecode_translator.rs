...
impl<'env> BoogieTranslator<'env> {
    ...
    fn extend_selected_theory(
        selected_theory: &mut MonoSlice,
        mono_info: &mono_analysis::MonoInfo,
        fun_target: &FunctionTarget<'_>,
        variant: &FunctionVariant,
        type_inst: &[Type],
    ) {
        let root = VerificationRoot {
            fun: fun_target.func_env.get_qualified_id(),
            variant: variant.clone(),
            inst: type_inst.to_vec(),
        };
        let slice = Self::root_slice(mono_info, &root);
        for (qid, insts) in &slice.structs {
            selected_theory
                .structs
                .entry(*qid)
                .or_default()
    ...
    fn collect_selected_theory(&mut self, mono_info: &mono_analysis::MonoInfo) {
        ...
        let mut selected_theory = MonoSlice::default();
        let empty = BTreeSet::new();
        for module_env in self.env.get_modules() {
            for fun_env in module_env.get_functions() {
                ...
                for (variant, fun_target) in self.targets.get_targets(&fun_env) {
                    if !self.is_verified(&variant, &fun_target) {
                        continue;
                    }
                    Self::extend_selected_theory(
                        &mut selected_theory,
                        mono_info,
                        &fun_target,
                        &variant,
        ...
        self.selected_theory = selected_theory;
    }
    ...
    fn generate_behavioral_predicate_evaluator(
        ...
        if fun_param_infos.is_empty() && struct_field_infos.is_empty() {
        ...
        for info in closure_infos {
            ...
            for kind in [BehaviorKind::RequiresOf, BehaviorKind::AbortsOf] {
                ...
                if kind == BehaviorKind::AbortsOf
                    && !has_complete_aborts
                    && (fun_env.is_opaque() || fun_env.is_native_or_intrinsic())
                {
                    emitln!(
                        self.writer,
                        "function {}({}): bool;",
            ...
            self.env.diag(
                ...
                &format!(
                    "generic type reflection in a function specification is not yet \
                     supported through behavioral predicates; weakening `{}`; see \
