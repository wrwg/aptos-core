    ...
    for target in targets.keys().collect_vec() {
        ...
        match (&target, target.get_env_state(env)) {
            ...
            (SpecBlock(sb_target), Spec(spec)) => {
                // Extract enclosing function if this spec block is for a function
                let enclosing_fun = match sb_target {
                    SpecBlockTarget::Function(mid, fid) => Some(mid.qualified(*fid)),
                    SpecBlockTarget::FunctionCode(mid, fid, _) => Some(mid.qualified(*fid)),
                    _ => None,
                };
                ...
                if let Some(fun_id) = enclosing_fun {
                    converter = converter.with_enclosing_fun(fun_id);
                }
...
///
/// Notice that `TempIndex` locals are only exist artificially during translation as
...
impl<'a> SpecConverter<'a> {
    ...
    fn generate_behavior_spec_fun(
        ...
        let (fn_arg_types, fn_result_types) = match param_type {
            Type::Fun(arg, result, _) => {
                let arg_types = match arg.as_ref() {
                    Type::Tuple(ts) => ts.clone(),
                    t => vec![t.clone()],
                };
                let result_types = match result.as_ref() {
                    Type::Tuple(ts) if ts.is_empty() => vec![],
                    Type::Tuple(ts) => ts.clone(),
                    t => vec![t.clone()],
                };
    ...
    fn reduce_behavior_predicate(
        ...
        let (translated, param_temps, ret_temps) = {
            ...
            let ret_types = target_fun_env.get_result_type();
            let ret_type_vec: Vec<Type> = match &ret_types {
                Type::Tuple(types) if types.is_empty() => vec![],
                Type::Tuple(types) => types.clone(),
                other => vec![other.clone()],
            };
            let ret_temps: Vec<TempIndex> = ret_type_vec
                .iter()
