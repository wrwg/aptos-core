...
use crate::{
    error::MempoolError, payload_manager::QuorumStoreProxy, state_replication::PayloadManager,
};
use anyhow::Result;
use aptos_types::{
...
pub struct MockPayloadManager {
    ...
    quorum_store_proxy: Option<QuorumStoreProxy>,
}

impl MockPayloadManager {
    pub fn new(consensus_to_quorum_store_sender: Option<mpsc::Sender<ConsensusRequest>>) -> Self {
        let quorum_store_proxy =
            consensus_to_quorum_store_sender.map(|s| QuorumStoreProxy::new(s, 1, 1));
        Self { quorum_store_proxy }
    }
}
