...
use consensus_types::{
    block::Block, common::TransactionSummary, executed_block::StateComputeResult,
};
...
impl TxnNotifier for MempoolNotifier {
    async fn notify_failed_txn(
        ...
        let req = TxnNotificationRequest::RejectNotification(rejected_txns, callback);

        // send to shared mempool
        self.executor_to_mempool_sender
            .clone()
            .try_send(req)
