...
fn test_consensus_events_rejected_txns() {
    ...
    let req = TxnNotificationRequest::RejectNotification(transactions, callback);
    let mut executor_sender = smp.executor_to_mempool_sender.clone();
    block_on(async {
        assert!(executor_sender.send(req).await.is_ok());
