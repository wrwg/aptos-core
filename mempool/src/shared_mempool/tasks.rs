...
use crate::{
    ...
    QuorumStoreRequest, QuorumStoreResponse, SubmissionStatus, TxnNotificationRequest,
    TxnNotificationResponse,
};
use anyhow::Result;
...
pub(crate) fn process_txn_notification_request<V: TransactionValidation>(
    smp: &SharedMempool<V>,
    req: TxnNotificationRequest,
) {
    // Start latency timer
    let start_time = Instant::now();
    debug!(
        LogSchema::event_log(LogEntry::TxnNotification, LogEvent::Received)
            .txn_notification_msg(&req)
    );

    let (resp, callback, counter_label) = match req {
        TxnNotificationRequest::RejectNotification(transactions, callback) => {
            ...
            (
                TxnNotificationResponse::CommitResponse(),
                callback,
                counters::COMMIT_CONSENSUS_LABEL,
            )
        }
