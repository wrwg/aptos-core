...
pub enum TxnNotificationRequest {
    /// Notifications about *rejected* committed txns.
    RejectNotification(
        // rejected transactions from consensus
        Vec<TransactionSummary>,
        // callback to respond to
        oneshot::Sender<Result<TxnNotificationResponse>>,
    ),
}

impl fmt::Display for TxnNotificationRequest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let payload = match self {
            TxnNotificationRequest::RejectNotification(rejected_txns, _) => {
...
pub enum TxnNotificationResponse {
